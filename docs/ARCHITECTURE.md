...
src/core/command.js  buildLlamaCppCommand(...) / buildOllamaCommand(...)
              ...
              ▼
src/main.js   reads form inputs → solve() → build*Command() → renders
              command, offload bar, readout, warning, methodology panel
```
...
- `src/main.js` — form state, backend toggle (llama.cpp/Ollama), the typed-command reveal
  animation, offload bar, won't-fit warning, clipboard copy, and the "why this estimate"
  methodology panel.
...
`test/solver.test.js` and `test/command.test.js` cover `src/core/*` directly — no DOM, no
mocking. `src/main.js` (DOM wiring) is not unit-tested; it's thin glue over the tested core
functions and is exercised manually per the design self-review in `docs/DESIGN.md`.
